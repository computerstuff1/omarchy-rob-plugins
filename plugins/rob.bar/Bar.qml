...
Item {
  ...
  // Rounded bar look: float the bar a small distance from the screen edges
  // and round its corners into a pill. The inset follows the shell's
  // panel-to-screen-edge gap so it tracks the theme; the radius rounds the
  // bar ends fully.
  readonly property int barInset: Math.max(4, Style.gapsOut)
  readonly property int barRadius: Math.round(barSize / 2)
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: (root.barHidden && root.position === "top" ? -root.barSize : 0) + root.barInset
      bottom: (root.barHidden && root.position === "bottom" ? -root.barSize : 0) + root.barInset
      left: (root.barHidden && root.position === "left" ? -root.barSize : 0) + root.barInset
      right: (root.barHidden && root.position === "right" ? -root.barSize : 0) + root.barInset
    }
    ...
    // The rounded pill behind the bar contents. The window is fully
    // transparent; only this surface paints, so the bar shows up rounded
    // and inset from the screen edges.
    BorderSurface {
      anchors.fill: parent
